...
async function setTime(targetDate) {
	...
	// 确保新时间戳大于当前时间戳
	const newTimestamp = Math.max(timestamp, currentBlock.timestamp + 1)
	...
	console.log("设置时间为:", new Date(block.timestamp * 1000).toLocaleString())
...
async function main() {
	...
	for (let i = 0; i < courses.length; i++) {
		...
		await time.increase(tiemincrease)
		if (i === 0) {
